...
## 项目概述
本项目旨在使用MongoDB作为后端框架，并结合Node.js，搭建一个用于管理课程资料的数据库及对应的管理系统。系统主要功能包括课程资料的导入，课程资料的增删改查，用户权限管理，以及数据的备份与恢复。
...
- 本项目使用的课程数据来源于[canvas网站](https://oc.sjtu.edu.cn)，课程信息可以以json文件的形式批量导入数据库
...
### 用户操作
- **创建用户**: `createUser(username, password, role)`
- **验证用户**: `authenticateUser(username, password)`
- **更新用户信息**: `updateUser(userId, updateData)`
- **删除用户**: `deleteUser(userId)`

### 课程操作
- **创建课程**: `createCourse(title, description)`
- **获取课程列表**: `getCourses()`
- **更新课程信息**: `updateCourse(courseId, updateData)`
- **删除课程**: `deleteCourse(courseId)`

### 资料操作
- **添加资料**: `addMaterial(name, type, url)`
- **获取资料列表**: `getMaterials()`
- **更新资料信息**: `updateMaterial(materialId, updateData)`
- **删除资料**: `deleteMaterial(materialId)`
